...
ARM_CFLAGS+=-std=c2x -g -Wall -Wextra -Werror -pedantic -fomit-frame-pointer -nostartfiles -ffreestanding $(ARM_CARCH) -O3
...
default: $(ARM_OBJS)
	$(ARM)gcc $(ARM_LDFLAGS) $(ARM_OBJS) -o $(ARM_BIN).elf -Wl,-Ttext,$(START_ADDRESS) -Wl,--section-start=.stack=0x0 -Wl,--section-ordering-file=sections.ld
...
qemu:
ifeq ($(RPI),1)
	qemu-system-arm -s -S -serial mon:stdio -M raspi1ap -smp 1 -m 512M -cpu arm1176 -kernel kernel.img -append "root=PARTUUID=d97f5830-02 rw console=ttyAMA0"
else ifeq ($(RPI),3)
	qemu-system-aarch64 -s -S -serial mon:stdio -M raspi2b -smp 4 -m 1G -cpu cortex-a53 -kernel kernel7.img -append "root=PARTUUID=d97f5830-02 rw console=ttyAMA0"
else ifeq ($(RPI),4)
	qemu-system-aarch64 -s -S -serial mon:stdio -M raspi4b -smp 4 -m 2G -cpu cortex-a72 -kernel kernel8.img -append "root=PARTUUID=d97f5830-02 rw console=ttyAMA0"
endif
